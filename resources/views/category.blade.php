        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            ...
            <div class="relative group overflow-hidden rounded-lg aspect-video cursor-pointer">
                <a href="{{ route('detail', $game->slug) }}">
                    <img src="{{ $game->cover_image ? \Illuminate\Support\Facades\Storage::url($game->cover_image) : 'https://placehold.co/600x400/333/666?text=' . urlencode($game->title) }}" alt="{{ $game->title }}" class="absolute inset-0 w-full h-full object-cover transition-transform duration-300 group-hover:scale-105">
