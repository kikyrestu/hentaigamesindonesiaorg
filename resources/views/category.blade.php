...
@section('title', 'Category - ' . $category->name . ' - Kimochi Gaming')

@section('content')
    <main class="container mx-auto px-4 py-8">
        
        <!-- Category Title -->
        <h2 class="text-2xl font-bold text-gray-200 mb-6">Category - {{ $category->name }}</h2>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            ...
            <div class="relative group overflow-hidden rounded-lg aspect-video cursor-pointer">
                <a href="{{ route('detail', $game->slug) }}">
                    <img src="{{ $game->thumbnail_image ? \Illuminate\Support\Facades\Storage::url($game->thumbnail_image) : ($game->cover_image ? \Illuminate\Support\Facades\Storage::url($game->cover_image) : 'https://placehold.co/600x400/333/666?text=' . urlencode($game->title)) }}" alt="{{ $game->title }}" class="absolute inset-0 w-full h-full object-cover transition-transform duration-300 group-hover:scale-105">
